...
- APIs for editing workflow and license have been changed. The API endpoint
  for editing content of the workflow now /edit (prefixed with /api/workflows).
  The endpoint for archiving or unarchiving a workflow or a license is
  /archived and the endpoint for enabling or disabling a workflow or a
  license is /enabled, prefixed with /api/workflows or /api/licenses,
  respectively.
