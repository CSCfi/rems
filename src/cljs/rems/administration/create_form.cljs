(ns rems.administration.create-form
  ...
  their answers and especially references between fields. The id can be given in the API,
  or it can be generated by the backend too. We also maintain a separate `:field/index`
  to automatically number fields for the user's benefit.
