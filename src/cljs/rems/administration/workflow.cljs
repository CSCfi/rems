...
;; raw sub, uses localization functions
(rf/reg-sub-raw
 ::disable-commands-table-rows
 (fn [db]
   (r/reaction
    (doall
     (for [[index value] (indexed (get-in @db [::workflow :workflow :disable-commands]))
...
(rf/reg-sub-raw
 ...
 (fn [db]
   (r/reaction
    (doall
     (for [[index value] (indexed (get-in @db [::workflow :workflow :processing-states]))]
