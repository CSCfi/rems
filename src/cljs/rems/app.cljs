...
(rf/reg-event-db
 ::on-request
 (fn [db [_ k]]
   (assoc-in db [::request k] true)))

(rf/reg-event-db
 ::on-request-finished
 (fn [db [_ k]]
   (dissoc-in db [::request k])))
...
(defn hook-browser-navigation! []
  (events/listen accountant/history
                 ...
                 (fn [^js event]
                   (rems.app-hooks/on-navigate (.-token event))))
  ...
  (events/listen accountant/history
                 HistoryEventType/NAVIGATE
                 (fn [_event]
                   (rf/dispatch [::user-triggered-navigation]))))
