...
;; This subscription calls the localization functions in rems.text, which implicitly subscribe to language.
;; Raw subscription is a way of telling re-frame "i know what i'm doing" because we create dynamic subscriptions.
(rf/reg-sub-raw
 ::table-rows
 (fn [db [_ apps-sub]]
   (let [apps (rf/subscribe [apps-sub])]
     (r/reaction
      (doall ; to realize all dynamic subscriptions
       (for [app @apps]
