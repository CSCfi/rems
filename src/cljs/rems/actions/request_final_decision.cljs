(ns rems.actions.request-final-decision
...
(def ^:private action-form-id "request-final-decision")
(def ^:private dropdown-id "request-final-decision-dropdown")

(rf/reg-event-fx
 ::send-request-decision
 (fn [_ [_ {:keys [deciders application-id comment on-finished]}]]
   (let [description [text :t.actions/request-final-decision]]
     (post! "/api/applications/request-final-decision"
...
(defn request-final-decision-action-button []
  [action-button {:id action-form-id
                  :text (text :t.actions/request-final-decision)
                  :on-click #(rf/dispatch [::open-form])}])

(defn request-final-decision-view
  [{:keys [selected-deciders potential-deciders comment on-set-comment on-set-deciders on-send]}]
  [action-form-view action-form-id
   (text :t.actions/request-final-decision)
...
(defn request-final-decision-form [application-id on-finished]
  (let [selected-deciders @(rf/subscribe [::selected-deciders])
    ...
    [request-final-decision-view {:selected-deciders selected-deciders
