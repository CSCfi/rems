  ...
  (let [id (:id (catalogue/create-catalogue-item! (merge {:form nil} ; SQL requires, API doesn't
        ...
        (doall
         (for [[langcode localization] localizations]
           (:id (catalogue/upsert-catalogue-item-localization! {:id id
...
(defn edit-catalogue-item! [{:keys [id localizations organization] :as item}]
  ...
  (when (:organization/id organization)
    (util/check-allowed-organization! organization)
    (catalogue/set-catalogue-item-organization! {:id id
                                                 :organization (:organization/id organization)}))
  (doseq [[langcode localization] localizations]
    (catalogue/upsert-catalogue-item-localization!
     (merge {:id id
             :langcode (name langcode)}
...
(defn change-form!
  ...
  (if (= (:formid item) form-id)
    ...
    (let [new-item (catalogue/create-catalogue-item! {:enabled true
                                                      :archived false
                                                      :form form-id
      ...
      (doseq [[langcode localization] (:localizations item)]
        (catalogue/upsert-catalogue-item-localization! {:id (:id new-item)
                                                        :langcode (name langcode)
                                                        :title (:title localization)
                                                        :infourl (:infourl localization)}))

      ;; hide the old catalogue item
      (catalogue/set-catalogue-item-enabled! {:id (:id item) :enabled false})
