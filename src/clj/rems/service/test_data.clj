...
(defn create-performance-test-data! []
  ...
  (let [start-time (System/currentTimeMillis)
        ...
        handlers-data (for [i (range handler-count)
                            :let [first-name (random-word)
                                  last-name (random-word)]]
                        {:userid (str "perf-test-handler-" i)
                         :name (str (str/capitalize first-name) " " (str/capitalize last-name))
                         :email (str first-name "." last-name "@perftester.org")})
