...
(deftest test-todo-commands
  (let [non-todo-commands
        ...
        all-commands (set application-schema/command-names)]
...
(deftest test-all-command-types-handled
  (is (= (set application-schema/command-names)
...
(defmethod command-handler :application.command/change-processing-state
  [cmd application injections]
  (add-comment-and-attachments cmd application injections
                               {:event/type :application.event/processing-state-changed
                                :event/public (:public cmd)
                                :application/processing-state {:processing-state/value (:processing-state cmd)}}))
...
(def ^:private validate-command-schema
  (s/validator application-schema/Command))

(defn- validate-command [cmd]
  (assert-ex (contains? application-schema/command-schemas (:type cmd))
