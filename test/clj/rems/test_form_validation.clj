(ns rems.test-form-validation
  ...
  (:require [clojure.test :refer :all]
            [rems.form-validation :refer [validate-fields-on-submit]]))

(deftest test-validate-fields-on-submit
  (testing "all fields filled"
    (is (nil? (validate-fields-on-submit [{:field/title "A"
  ...
  (testing "labels and headers are always effectively optional"
    (is (nil? (validate-fields-on-submit [{:field/type :label
                                 :field/optional       false
                                 :field/visible        true
                                 :field/value          ""}
                                {:field/type :label
  ...
  (testing "error: field required"
    (is (= [{:type :t.form.validation/required :field-id "2"}
           ...
           (validate-fields-on-submit [{:field/id "1"
                              :field/optional     true
                              :field/visible      true
                              :field/value        nil}
                             {:field/id "2"
