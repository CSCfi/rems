      ...
      (let [res-id (test-helpers/create-resource! {:resource-ext-id "urn:nbn:fi:lb-201403262"})
        ...
        (let [api-key "42"
          ...
          (testing "export entitlements csv"
            (testing "as handler and reporter"
              (let [get-csv (fn [user]
                              (-> (request :get (str "/api/entitlements/csv"))
                ...
                (is (= [["resource" "application" "user" "start"]
                        ["urn:nbn:fi:lb-201403262" (str app-id) "alice" "2010-01-01 00:00"]
                        ["urn:nbn:fi:lb-201403262" (str expired-app-id) "alice" "2010-01-01 00:00"]
                        ["urn:nbn:fi:lb-201403262" (str malice-app-id) "malice" "2010-01-01 00:00"]]
                       (get-csv "developer")
                       (get-csv "reporter")))))
            (testing "without authentication"
              (let [response (-> (request :get (str "/api/entitlements/csv"))
                                 handler)
                    body (read-body response)]
                (is (= "unauthorized" body))))
            (testing "without proper role"
              (let [get-response (fn [user]
                                   (-> (request :get (str "/api/entitlements/csv"))
                                       (authenticate api-key user)
                                       handler
                                       read-body))]
