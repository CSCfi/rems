...
(deftest test-get-catalogue-items
  ...
  (testing "with two items"
    (let [item1 (test-data-functions/create-catalogue-item! {})
          item2 (test-data-functions/create-catalogue-item! {})]
...
(deftest test-multi-applications
  (test-data-functions/create-user! {:eppn "test-user" :mail "test-user@test.com" :commonName "Test-user"})
  (test-data-functions/create-user! {:eppn "handler" :mail "handler@test.com" :commonName "Handler"})
  (let [applicant "test-user"
        wfid (test-data-functions/create-workflow! {:handlers ["handler"]})
        res1 (test-data-functions/create-resource! {:resource-ext-id "resid111"})
        res2 (test-data-functions/create-resource! {:resource-ext-id "resid222"})
        form-id (test-data-functions/create-form! {})
        item1 (test-data-functions/create-catalogue-item! {:form-id form-id :resource-id res1 :workflow-id wfid})
        item2 (test-data-functions/create-catalogue-item! {:form-id form-id :resource-id res2 :workflow-id wfid})
        app-id (test-data-functions/create-application! {:catalogue-item-ids [item1 item2]
                                               :actor applicant})]
...
(deftest test-get-entitlements-for-export
  (test-data-functions/create-user! {:eppn "handler" :mail "handler@test.com" :commonName "Handler"})
  (test-data-functions/create-user! {:eppn "jack" :mail "jack@test.com" :commonName "Jack"})
  (test-data-functions/create-user! {:eppn "jill" :mail "jill@test.com" :commonName "Jill"})
  (let [wf (test-data-functions/create-workflow! {:handlers ["handler"]})
        form-id (test-data-functions/create-form! {})
        res1 (test-data-functions/create-resource! {:resource-ext-id "resource1"})
        res2 (test-data-functions/create-resource! {:resource-ext-id "resource2"})
        item1 (test-data-functions/create-catalogue-item! {:form-id form-id :resource-id res1 :workflow-id wf})
        item2 (test-data-functions/create-catalogue-item! {:form-id form-id :resource-id res2 :workflow-id wf})
        jack-app (test-data-functions/create-application! {:actor "jack" :catalogue-item-ids [item1]})
        jill-app (test-data-functions/create-application! {:actor "jill" :catalogue-item-ids [item1 item2]})]
    (test-data-functions/command! {:type :application.command/submit
