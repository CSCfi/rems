...
(deftest test-outbox
  (let [deadline (-> (time/now) (.plusMinutes 1))
        id1 (rems.db.outbox/put! {:outbox/type :email
                                  :outbox/email {:to-user "user 1"
                                                 :subject "subject 1"
                                                 :body "body 1"}
                                  :outbox/deadline deadline})
        id2 (rems.db.outbox/put! {:outbox/type :email
    ...
    (testing "get by id"
      (let [emails (rems.db.outbox/get-entries {:ids [id1]})
            email (rems.db.outbox/get-entry-by-id id1)]
        (is (= 1 (count emails)))
        (is (= emails [email]))
      ...
      (is (= [{:outbox/id id1
              ...
              {:outbox/id id2
               :outbox/email {:to "user2@example.com"
                              :subject "subject 2"
                              :body "body 2"}}]
             (->> (rems.db.outbox/get-entries)
                  (map #(select-keys % [:outbox/id :outbox/email]))
                  (sort-by :outbox/id)))))

    (testing "get by type"
      (is (= [id1 id2]
             (->> (rems.db.outbox/get-entries {:type :email})
                  (map :outbox/id)
                  sort)))
      (is (empty? (rems.db.outbox/get-entries {:type :foobar}))))

    (testing "get due now, before attempt"
      (is (= [id1 id2]
             (->> (rems.db.outbox/get-entries {:due-now? true})
                  (map :outbox/id)
                  sort))))

    (testing "attempt failed"
      (let [email (rems.db.outbox/get-entry-by-id id1)]
        (rems.db.outbox/attempt-failed! email "the error message"))
      (let [email (rems.db.outbox/get-entry-by-id id1)
            unrelated (rems.db.outbox/get-entry-by-id id2)]
        (is (= {:outbox/id id1
    ...
    (testing "attempt succeeded"
      (rems.db.outbox/attempt-succeeded! id1)
      (is (= [id2]
             (->> (rems.db.outbox/get-entries)
                  (map :outbox/id)
                  sort))
