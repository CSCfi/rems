...
(defn ->add-to-cart-button-q
  ([resource-name] (->add-to-cart-button-q resource-name :catalogue))
  ([resource-name selector]
   [selector
    {:fn/text resource-name}
    {:xpath "./ancestor::tr"}
    {:css ".add-to-cart"}])
  ([resource-name selector & selectors]
   (mapv (partial ->add-to-cart-button-q resource-name) (cons selector selectors))))

(defn ->remove-from-cart-button-q
  ([resource-name] (->remove-from-cart-button-q resource-name :catalogue))
  ([resource-name selector]
   [selector
    {:fn/text resource-name}
    {:xpath "./ancestor::tr"}
    {:css ".remove-from-cart"}])
  ([resource-name selector & selectors]
   (mapv (partial ->remove-from-cart-button-q resource-name) (cons selector selectors))))

(defn add-to-cart [resource-name]
  (btu/scroll-and-click (->add-to-cart-button-q resource-name)))

(defn remove-from-cart [resource-name]
  (btu/scroll-and-click (->remove-from-cart-button-q resource-name)))

(defn click-cart-apply [q]
  (btu/scroll-and-click q)
  (is (btu/eventually-visible? {:tag :h1 :fn/has-text "Application"}))
  (btu/wait-page-loaded)
  (btu/screenshot "application-page")
  (btu/gather-axe-results "application-page"))

(defn click-cart-apply-bundle []
  (click-cart-apply [{:css "table.cart"}
                     {:fn/has-text "Apply for"}
                     {:xpath "./ancestor::tr"}
                     {:css ".apply-for-catalogue-items"}]))

(defn click-cart-apply-single []
  (click-cart-apply [{:css ".rems-table.cart"}
                     {:css ".apply-for-catalogue-items"}]))
...
(defn select-option-id [id option]
  (btu/wait-for-idle)
  (btu/wait-enabled {:id id})
  (btu/fill {:id id} option etaoin.keys/enter)) ; as above
  ...
  (btu/with-postmortem
    ...
    (testing "create application"
      ...
      (let [application (get-application-from-api (btu/context-getx :application-id) "alice")
        ...
        (open-collapsible :header-collapsible) ; show application state details
...
(deftest test-applicant-member-remove-action
  ...
  (btu/with-postmortem
    ...
    (testing "remove second member jade"
      ...
      (is (not (btu/visible? :member1-info-operations-remove-action-button)))
      (open-collapsible :applicants-info-collapsible)
      ...
      (Thread/sleep 500)
      ;; first event
      (is (btu/visible? [{:css ".event" :index 1} {:css ".event-description" :fn/text "Alice Applicant removed Jade Jenner from the application."}]))
      (is (btu/visible? [{:css ".event" :index 1} {:css ".event-comment" :fn/text "not in research group anymore"}])))))
  ...
  (btu/with-postmortem
    ...
    (testing "handler should see the applicant info"
      ...
      (is (= {"Name" "Alice Applicant"
             ...
             (slurp-fields :applicants-info-collapsible))))
...
(deftest test-processed-applications-and-paging
  ...
  (btu/with-postmortem
    ...
    (testing "with 100+ processed applications"
      ...
      (open-collapsible :handled-applications-collapsible)
      (btu/wait-visible {:fn/text "Showing the first 50 rows only."})
      (btu/screenshot "processed-applications-opened")
      (btu/scroll-and-click [:handled-applications-collapsible {:fn/text "Show all rows"}])
...
(deftest test-catalogue-item-hierarchy
  (btu/with-postmortem
    (btu/context-assoc! :organization-id (str "organization " (btu/get-seed))
                        :organization-name (str "Organization " (btu/get-seed)))
    (btu/context-assoc! :organization (test-helpers/create-organization!
                                       {:organization/id (btu/context-getx :organization-id)
                                        :organization/short-name (->> (str/split (btu/get-seed) #" ")
                                                                      (transduce (comp (map first) (map str) (interpose "-")) str "ORG ")
                                                                      test-helpers/make-localized)
                                        :organization/name (test-helpers/make-localized (btu/context-getx :organization-name))}))
    ...
    (btu/context-assoc! :resource-1 (test-helpers/create-resource!
                                     {:resource-ext-id (str "test-catalogue-item-hierarchy" (btu/get-seed) " resource-1")
                                      :organization {:organization/id (btu/context-getx :organization-id)}})
                        :resource-2 (test-helpers/create-resource!
                                     {:resource-ext-id (str "test-catalogue-item-hierarchy" (btu/get-seed) " resource-2")
                                      :organization {:organization/id (btu/context-getx :organization-id)}})
                        :resource-3 (test-helpers/create-resource!
                                     {:resource-ext-id (str "test-catalogue-item-hierarchy" (btu/get-seed) " resource-3")
                                      :organization {:organization/id (btu/context-getx :organization-id)}})
    ...
    (btu/context-assoc! :parent-1-title-en (:en (test-helpers/make-localized (btu/context-getx :parent-1-name)))
                        ...
                        :child-4-title-en (:en (test-helpers/make-localized (btu/context-getx :child-4-name))))
    (btu/context-assoc! :child-1-id (test-helpers/create-catalogue-item!
                                     {:title (test-helpers/make-localized (btu/context-getx :child-1-name))
    ...
    (testing "create catalogue item with child"
      ...
      (btu/wait-page-loaded)
      (select-option "Organization" (btu/context-getx :organization-name))
      (fill-localized-form-field "Title" "EN" (str (btu/context-getx :parent-1-name) " EN"))
      (fill-localized-form-field "Title" "FI" (str (btu/context-getx :parent-1-name) " FI"))
      (fill-localized-form-field "Title" "SV" (str (btu/context-getx :parent-1-name) " SV"))
      (select-option "Workflow" (btu/context-getx :workflow-name))
      (select-option "Resource" (btu/context-getx :resource-1))
      (select-option "Form" (btu/context-getx :form))
      (select-option "Complementary items" (btu/context-getx :child-1-name))
      (btu/screenshot "about-to-create-catalogue-item")
      (btu/scroll-and-click :save)
      (wait-page-title "Catalogue item – REMS")
      (btu/wait-page-loaded)
      ...
      (is (str/includes? (btu/get-element-text {:css ".alert-success"}) "Success"))
      (is (= (str (btu/context-getx :child-1-id))
             (get (slurp-fields :catalogue-item) "Complementary items")))
      (enable-catalogue-item (btu/context-getx :parent-1-name)))
    ...
    (testing "add child item"
      ...
      (select-option "Complementary items" (btu/context-getx :child-2-name))
      ...
      (testing "catalogue item page displays child info"
        (btu/wait-page-loaded)
        (is (= (str (btu/context-getx :child-2-id))
               (get (slurp-fields :catalogue-item) "Complementary items")))))
    ...
    (testing "add second child item"
      ...
      (is (= (str (btu/context-getx :child-2-name) " EN")
             (get (slurp-fields :catalogue-item-editor) "Complementary items")))
      (select-option "Complementary items" (btu/context-getx :child-3-name))
      (is (match? (m/in-any-order  [(btu/context-getx :child-2-title-en)
      ...
      (testing "catalogue item page displays child info"
        (btu/wait-page-loaded)
        (is (match? (m/in-any-order [(str (btu/context-getx :child-2-id))
                                     (str (btu/context-getx :child-3-id))])
                    (-> (slurp-fields :catalogue-item)
                        (get "Complementary items")
                        (str/split #", "))))))

    (testing "when editing child item"
      (doseq [child [(btu/context-getx :child-2-id) (btu/context-getx :child-3-id)]]
        ...
        (is (= (str (btu/context-getx :parent-2-id))
    ...
    (testing "remove child item 2"
      ...
      (remove-option "Complementary items" (btu/context-getx :child-2-name))
      (is (= (btu/context-getx :child-3-title-en)
             (get (slurp-fields :catalogue-item-editor) "Complementary items")))
      (btu/screenshot "after-removing-child-item")
      (btu/scroll-and-click :save)
      (is (btu/eventually-visible? {:css ".alert-success"}))
      (testing "child 2 is removed and child 3 visible"
        (is (= (str (btu/context-getx :child-3-id))
               (get (slurp-fields :catalogue-item) "Complementary items")))))
    ...
    (testing "as an applicant"
      ...
      (login-as "alice")
      (go-to-catalogue)
      (testing "can see the items"
        (let [item-titles [(btu/context-getx :parent-1-title-en)
                           (btu/context-getx :child-1-title-en)
                           (btu/context-getx :parent-2-title-en)
                           (btu/context-getx :child-2-title-en)
                           (btu/context-getx :child-3-title-en)]]
          (is (match? (m/in-any-order item-titles)
                      (into []
                            (keep (comp (set item-titles) #(get % "name")))
                            (slurp-rows :catalogue))))))
      ...
      (testing "with empty cart"
        ...
        (is (not (btu/disabled? (->add-to-cart-button-q (btu/context-getx :child-2-title-en))))
            "should not be disabled with an item not belonging to hierarchy")
        (is (btu/disabled? (->add-to-cart-button-q (btu/context-getx :child-3-title-en))))
        (is (btu/disabled? (->add-to-cart-button-q (btu/context-getx :child-3-title-en) :catalogue-tree))))
      ...
      (testing "with parent in cart"
        ...
        (is (not (btu/disabled? (->remove-from-cart-button-q (btu/context-getx :parent-1-title-en) :catalogue-tree)))
            "can remove parent")
        (is (not (btu/disabled? (->add-to-cart-button-q (btu/context-getx :child-1-title-en))))
            "can add child")
        (is (not (btu/disabled? (->add-to-cart-button-q (btu/context-getx :child-1-title-en) :catalogue-tree)))
            "can add child"))
      ...
      (testing "with child in cart"
        ...
        (is (btu/disabled? (->remove-from-cart-button-q (btu/context-getx :parent-1-title-en) :cart))
            "cannot remove parent")
        (is (btu/disabled? (->remove-from-cart-button-q (btu/context-getx :parent-1-title-en) :catalogue-tree))
            "cannot remove parent")
        (is (not (btu/disabled? (->remove-from-cart-button-q (btu/context-getx :child-1-title-en))))
            "can remove child")
        (is (not (btu/disabled? (->remove-from-cart-button-q (btu/context-getx :child-1-title-en) :cart)))
            "can remove child")
        (is (not (btu/disabled? (->remove-from-cart-button-q (btu/context-getx :child-1-title-en) :catalogue-tree)))
            "can remove child"))
      ...
      (testing "with child removed from cart"
        ...
        (is (not (btu/disabled? (->remove-from-cart-button-q (btu/context-getx :parent-1-title-en) :catalogue-tree)))
            "can remove parent again")
        (remove-from-cart (btu/context-getx :parent-1-title-en)))
        ...
        (testing "can add child item to cart on it's own"
          ...
          (is (= [(btu/context-getx :child-4-title-en)]
                 (into []
                       ...
                       (slurp-cart))))
          (remove-from-cart (btu/context-getx :child-4-title-en)))

        (testing "can add and remove parent"
          (add-to-cart (btu/context-getx :child-4-title-en))
          (add-to-cart (btu/context-getx :parent-2-title-en))
          (btu/wait-page-loaded)
          (btu/screenshot "child-and-parent-added")
          (is (match? (m/in-any-order [(btu/context-getx :parent-2-title-en)
                                       (btu/context-getx :child-4-title-en)])
                      (into []
                            (mapcat (comp vals #(dissoc % "commands")))
                            (butlast (slurp-cart)))))
          (is (not (btu/disabled? (->remove-from-cart-button-q (btu/context-getx :parent-2-title-en)))))
          (is (not (btu/disabled? (->remove-from-cart-button-q (btu/context-getx :parent-2-title-en) :cart))))
          (is (not (btu/disabled? (->remove-from-cart-button-q (btu/context-getx :parent-2-title-en) :catalogue-tree))))
          (btu/screenshot "add-and-remove-parent-2")
          (remove-from-cart (btu/context-getx :parent-2-title-en))
          (remove-from-cart (btu/context-getx :child-4-title-en)))

        (testing "can apply for child item on it's own"
          (add-to-cart (btu/context-getx :child-4-title-en))
          (btu/wait-page-loaded)
          (is (= [(btu/context-getx :child-4-title-en)]
                 (into []
                       (mapcat (comp vals #(dissoc % "commands")))
                       (slurp-cart))))
          (btu/screenshot "about-to-apply-for-child-item")
          (click-cart-apply-single)
          (wait-page-title (->> (get-application-from-api (get-application-id) "alice")
                                :application/external-id
                                (format "Application %s – REMS")))
          (btu/screenshot "draft-application-for-child-item")
          (is (= [(btu/context-getx :child-4-title-en)]
                 (into []
                       (map btu/value-of-el)
                       (-> (btu/query [{:class :application-resources}])
    ...
    (testing "invited user cannot join application of complementary item without also having entitlement to top-level item"
      ...
      (testing "cannot change resource to a complementary item, when invited member doesn't have entitlement to it's parent"
        ...
        (btu/scroll-and-click :change-resources-action-button)
        (btu/wait-page-loaded)
        (select-option-id :change-resources-dropdown (btu/context-getx :child-5-name))
        (btu/screenshot "about-to-change-resources")
        (btu/scroll-and-click :change-resources)
        (btu/wait-page-loaded)
        (is (btu/eventually-visible? [:flash-message-change-resources]))
        (is (= (str "Missing top-level item: " (btu/context-getx :parent-3-id))
               (btu/get-element-text [:flash-message-change-resources]))
            (str "flash message contains parent id: " (btu/context-getx :parent-3-id)))
        (btu/screenshot "after-change-resources")
        (btu/scroll-and-click :cancel-change-resources)
        (btu/scroll-and-click :change-resources-action-button)
        (select-option-id :change-resources-dropdown (btu/context-getx :parent-3-name))
        (btu/wait-page-loaded)
        (btu/screenshot "about-to-change-resources")
        (btu/scroll-and-click :change-resources)
        (btu/wait-page-loaded)
...
(deftest test-form-editor
  (btu/context-dissoc! :field-id :field-index) ; empty form
  ...
  (btu/with-postmortem
    ...
    (testing "create form"
      ...
      (testing "add and remove a field"
        ...
        (btu/wait-has-alert)
        (btu/accept-alert)
        (btu/wait-invisible (field-editor))
        (btu/context-dissoc! :field-id :field-index))
      ...
      (testing "create all field types"
        ...
        (testing "create multi-select field"
          ...
          (testing "create three options"
            (doseq [i (range 3)
                    :let [option-id (keyword (format "options-%s-key" i))
        ...
        (testing "create table field"
          ...
          (testing "create three columns and fill localized fields"
            (doseq [i (range 3)
        ...
        (testing "create attachment field"
          ...
          (testing "attachment has extra info in preview"
            ...
            (is (not (btu/visible? {:id (btu/context-getx ::attachment-collapsible)})))
            (btu/scroll-and-click {:css (aria-controls (btu/context-getx ::attachment-collapsible))})
        ...
        (testing "create header field"
          ...
          (testing "fill localized title"
            ...
            (is (btu/eventually-visible? [(field-preview) {:tag :h3 :fn/text "Header (EN)"}])))))
    ...
    (testing "view form"
      ...
      (testing "preview"
        ...
        (is (= ["Text field (EN) (max 127 characters) (optional)"
                ...
                "Label (EN)"
                "Header (EN)"]
               (->> (btu/query-all [:preview-form-contents {:class :field-preview}])
                    (filter btu/visible-el?)
                    (map btu/get-element-text-el)
                    (map (comp first str/split-lines))))))

      (testing "info collapse can be toggled"
        (btu/context-assoc! ::info-collapsible (btu/get-element-attr {:css ".info-collapsible-toggle[aria-controls]"} "aria-controls"))
        (btu/context-assoc! ::info-collapsible-toggle (str ".info-collapsible-toggle" (aria-controls (btu/context-getx ::info-collapsible))))
        (testing "open collapse"
          (is (not (btu/visible? {:id (btu/context-getx ::info-collapsible)})))
          (is (not (btu/visible? {:tag :div :fn/has-text "Text field description (EN)"})))
          (btu/scroll-and-click {:css (btu/context-getx ::info-collapsible-toggle)})
          (is (btu/eventually-visible? {:id (btu/context-getx ::info-collapsible)}))
          (is (btu/visible? {:tag :div :fn/has-text "Text field description (EN)"})))
        (testing "close collapse"
          (btu/scroll-and-click {:css (btu/context-getx ::info-collapsible-toggle)})
          (btu/wait-invisible {:tag :div :fn/has-text "Text field description (EN)"})
          (btu/wait-predicate #(not (btu/visible? {:tag :div :fn/has-text "Text field description (EN)"})) {:timeout 30}))
        (testing "change language and re-open collapse"
          (change-language :fi)
          (is (not (btu/visible? {:id (btu/context-getx ::info-collapsible)})))
          (is (not (btu/visible? {:tag :div :fn/has-text "Text field description (FI)"})))
          (btu/scroll-and-click {:css (btu/context-getx ::info-collapsible-toggle)})
          (is (btu/eventually-visible? {:id (btu/context-getx ::info-collapsible)}))
          (is (btu/visible? {:tag :div :fn/has-text "Text field description (FI)"})))))
    ...
    (testing "edit form"
      ...
      (testing "edit description field"
        (btu/scroll-and-click {:fn/text "Edit"})
        (btu/wait-page-loaded)
        (wait-page-title "Edit form – REMS")
        ...
        (testing "check that error message is present on field empty"
          (btu/wait-page-loaded)
          (wait-page-title "Edit form – REMS")
          (is (btu/visible? (merge (field-component :info-text-sv) {:fn/has-class :is-invalid})))
        ...
        (let [form-id (Integer/parseInt (last (str/split (btu/get-url) #"/")))]
          (is (= {:form/id form-id
                  ...
                  :form/fields [{:field/title {:fi "Description (FI)" :en "Description (EN)" :sv "Description (SV)"}
                                 :field/info-text {:en "Description (EN)", :fi "Description (FI)", :sv "Description (SV)"}
                                 :field/type "description"
                                 :field/id "fld13"
                                 :field/max-length nil
                                 :field/optional false}
                                {:field/placeholder {:fi "Text field placeholder (FI)" :en "Text field placeholder (EN)" :sv "Text field placeholder (SV)"}
                                 :field/title {:fi "Text field (FI)" :en "Text field (EN)" :sv "Text field (SV)"}
                                 :field/info-text {:fi "Text field description (FI)" :en "Text field description (EN)" :sv "Text field description (SV)"}
                                 :field/type "text"
                                 :field/id "fld1"
                                 :field/max-length 127
                                 :field/optional true}
                                {:field/placeholder {:fi "Text area placeholder (FI)" :en "Text area placeholder (EN)" :sv "Text area placeholder (SV)"}
                                 :field/title {:fi "Text area (FI)" :en "Text area (EN)" :sv "Text area (SV)"}
                                 :field/info-text {:fi "Text area description (FI)" :en "Text area description (EN)" :sv "Text area description (SV)"}
                                 :field/type "texta"
                                 :field/id "fld2"
                                 :field/max-length 127
                                 :field/optional true}
                                {:field/title {:fi "Option list (FI)" :en "Option list (EN)" :sv "Option list (SV)"}
                                 :field/info-text {:en "Option description (EN)" :fi "Option description (FI)" :sv "Option description (SV)"}
                                 :field/type "option"
                                 :field/id "fld3"
                                ...
                                {:field/columns [{:key "table-column-0"
                                 ...
                                 :field/type "table"}
                                {:field/title {:fi "Date (FI)" :en "Date (EN)" :sv "Date (SV)"}
                                 :field/info-text {:en "Date description (EN)", :fi "Date description (FI)", :sv "Date description (SV)"}
                                 :field/type "date"
                                 :field/id "fld6"
                                 :field/optional false}
                                {:field/title {:fi "Email (FI)" :en "Email (EN)" :sv "Email (SV)"}
                                 :field/info-text {:en "Email description (EN)", :fi "Email description (FI)", :sv "Email description (SV)"}
                                 :field/type "email"
                                 :field/id "fld7"
                                 :field/optional false}
                                {:field/title {:fi "Phone number (FI)" :en "Phone number (EN)" :sv "Phone number (SV)"}
                                 :field/info-text {:en "Phone number description (EN)", :fi "Phone number description (FI)", :sv "Phone number description (SV)"}
                                 :field/type "phone-number"
                                 :field/id "fld8"
                                 :field/optional false}
                                {:field/title {:fi "IP address (FI)" :en "IP address (EN)" :sv "IP address (SV)"}
                                 :field/info-text {:en "IP address description (EN)", :fi "IP address description (FI)", :sv "IP address description (SV)"}
                                 :field/type "ip-address"
                                 :field/id "fld9"
                                 :field/optional false}
                                {:field/title {:fi "Attachment (FI)" :en "Attachment (EN)" :sv "Attachment (SV)"}
                                 :field/info-text {:en "Attachment description (EN)", :fi "Attachment description (FI)", :sv "Attachment description (SV)"}
    ...
    (testing "create form"
      ...
      (testing "create conditional text field"
        ...
        (testing "set visibility to only if field 1 has true selected"
          ...
          (btu/fill (field-component :visibility-field) "Field 1: Option (EN)\n")
          (is (btu/eventually-visible? (field-component :visibility-value)))
      ...
      (testing "create conditional email field"
        ...
        (testing "set visibility to only if field 2 has X,Z selected"
          ...
          (btu/fill (field-component :visibility-type) "Only if\n")
          (is (btu/eventually-visible? (field-component :visibility-field)))
          (btu/fill (field-component :visibility-field) "Field 2: Multiselect (EN)\n")
          (is (btu/eventually-visible? (field-component :visibility-value)))
          (btu/fill (field-component :visibility-value) "X\n")
          (btu/fill (field-component :visibility-value) "Z\n")))
