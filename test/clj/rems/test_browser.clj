...
(defn login-as [driver username]
  (btu/set-window-size driver 1400 7000) ; big enough to show the whole page in the screenshots
  (btu/go driver (btu/get-server-url))
  (btu/screenshot driver (io/file btu/reporting-dir "landing-page.png"))
  (btu/scroll-and-click driver {:css ".login-btn"})
  (btu/screenshot driver (io/file btu/reporting-dir "login-page.png"))
  (btu/scroll-and-click driver [{:class "users"} {:tag :a :fn/text username}])
  (btu/wait-visible driver :logout)
  (btu/screenshot driver (io/file btu/reporting-dir "logged-in.png")))

(defn logout [driver]
  (btu/scroll-and-click driver :logout)
  (btu/wait-visible driver {:css ".login-component"}))
...
(defn go-to-catalogue [driver]
  (click-navigation-menu driver "Catalogue")
  (btu/wait-visible driver {:tag :h1 :fn/text "Catalogue"})
  (btu/wait-page-loaded driver)
  (btu/screenshot driver (io/file btu/reporting-dir "catalogue-page.png")))

(defn go-to-applications [driver]
  (click-navigation-menu driver "Applications")
  (btu/wait-visible driver {:tag :h1 :fn/text "Applications"})
  (btu/wait-page-loaded driver)
  (btu/screenshot driver (io/file btu/reporting-dir "applications-page.png")))

(defn go-to-admin-licenses [driver]
  (click-administration-menu driver "Licenses")
  (btu/wait-visible driver {:tag :h1 :fn/text "Licenses"})
  (btu/wait-page-loaded driver)
  (btu/screenshot driver (io/file btu/reporting-dir "administration-licenses-page.png")))

(defn go-to-admin-resources [driver]
  (click-administration-menu driver "Resources")
  (btu/wait-visible driver {:tag :h1 :fn/text "Resources"})
  (btu/wait-page-loaded driver)
  (btu/screenshot driver (io/file btu/reporting-dir "administration-resources-page.png")))
...
(defn apply-for-resource [driver resource-name]
  (btu/scroll-and-click driver [{:css "table.cart"}
                                {:fn/text resource-name}
                                {:xpath "./ancestor::tr"}
                                {:css ".apply-for-catalogue-items"}])
  (btu/wait-visible driver {:tag :h1 :fn/has-text "Application"})
  (btu/wait-page-loaded driver)
  (btu/screenshot driver (io/file btu/reporting-dir "application-page.png")))
...
(defn accept-licenses [driver]
  (btu/scroll-and-click driver :accept-licenses-button)
  (btu/wait-visible driver :has-accepted-licenses))

(defn send-application [driver]
  (btu/scroll-and-click driver :submit)
  (btu/wait-visible driver :status-success)
  (btu/wait-has-class driver :apply-phase "completed"))
  ...
  (let [driver (btu/get-driver)]
    (btu/with-postmortem driver {:dir btu/reporting-dir}
      ...
      (testing "create application"
        (go-to-catalogue driver)
        (add-to-cart driver "Default workflow")
        (apply-for-resource driver "Default workflow")
        ...
        (let [application-id (get-application-id driver)
          ...
          (fill-form-field driver "Application title field" "Test name")
          (fill-form-field driver "Text field" "Test")
          (fill-form-field driver "Text area" "Test2")
          (set-date driver "Date field" "2050-01-02")
          (fill-form-field driver "Email field" "user@example.com")
          (btu/upload-file driver attachment-field-selector "test-data/test.txt")
          (btu/wait-predicate driver #(= ["test.txt"] (get-attachments driver)))

          (is (not (btu/field-visible? driver "Conditional field"))
              "Conditional field is not visible before selecting option")

          (select-option driver "Option list" "First option")
          (btu/wait-predicate driver #(btu/field-visible? driver "Conditional field"))
          (fill-form-field driver "Conditional field" "Conditional")
          ;; pick two options for the multi-select field:
          (btu/check-box driver "Option2")
          (btu/check-box driver "Option3")
          ;; leave "Text field with max length" empty
          ;; leave "Text are with max length" empty

          (accept-licenses driver)
          (send-application driver)
  ...
  (let [driver (btu/get-driver)
    ...
    (btu/with-postmortem driver {:dir btu/reporting-dir}
      ...
      (testing "add a comment and two attachments"
        (btu/wait-visible driver :comment-approve-reject)
        (btu/fill-human driver :comment-approve-reject "this is a comment")
        (btu/upload-file driver :upload-approve-reject-input "test-data/test.txt")
        (btu/wait-visible driver [{:css "a.attachment-link"}])
        (btu/upload-file driver :upload-approve-reject-input "test-data/test-fi.txt")
  ...
  (let [driver (btu/get-driver)]
    (btu/with-postmortem driver {:dir btu/reporting-dir}
      (testing "default language is English"
        (btu/go driver (btu/get-server-url))
        (btu/wait-visible driver {:tag :h1 :fn/text "Welcome to REMS"})
        (login-as driver "alice")
        (btu/wait-visible driver {:tag :h1 :fn/text "Catalogue"})
        (btu/wait-page-loaded driver))
      ...
      (testing "changing language while logged out"
        (logout driver)
        (btu/wait-visible driver {:tag :h1 :fn/text "Welcome to REMS"})
        (change-language driver :fi)
        (btu/wait-visible driver {:tag :h1 :fn/text "Tervetuloa REMSiin"}))

      (testing "changed language must persist after login"
        (login-as driver "alice")
        (btu/wait-visible driver {:tag :h1 :fn/text "Aineistoluettelo"})
        (btu/wait-page-loaded driver))
...
(defn create-license [driver]
  (btu/with-postmortem driver {:dir btu/reporting-dir}
    (go-to-admin-licenses driver)
    (btu/scroll-and-click driver :create-license)
    (btu/wait-visible driver {:tag :h1 :fn/text "Create license"})
    (select-option driver "Organization" "nbn")
    (btu/scroll-and-click driver :licensetype-link)
    (fill-form-field driver "License name" (str (:license-name @btu/test-context) " EN") {:index 1})
    (fill-form-field driver "License link" "https://www.csc.fi/home" {:index 1})
    (fill-form-field driver "License name" (str (:license-name @btu/test-context) " FI") {:index 2})
    (fill-form-field driver "License link" "https://www.csc.fi/etusivu" {:index 2})
    (fill-form-field driver "License name" (str (:license-name @btu/test-context) " SV") {:index 3})
    (fill-form-field driver "License link" "https://www.csc.fi/home" {:index 3})
    (btu/screenshot driver (io/file btu/reporting-dir "about-to-create-license.png"))
    (btu/scroll-and-click driver :save)
    (btu/wait-visible driver {:tag :h1 :fn/text "License"})
    (btu/wait-page-loaded driver)
    (btu/screenshot driver (io/file btu/reporting-dir "created-license.png"))
...
(defn create-resource [driver]
  (btu/with-postmortem driver {:dir btu/reporting-dir}
    (go-to-admin-resources driver)
    (btu/scroll-and-click driver :create-resource)
    (btu/wait-visible driver {:tag :h1 :fn/text "Create resource"})
    (select-option driver "Organization" "nbn")
    (fill-form-field driver "Resource identifier" (:resid @btu/test-context))
    (select-option driver "License" (str (:license-name @btu/test-context) " EN"))
    (btu/screenshot driver (io/file btu/reporting-dir "about-to-create-resource.png"))
    (btu/scroll-and-click driver :save)
    (btu/wait-visible driver {:tag :h1 :fn/text "Resource"})
    (btu/wait-page-loaded driver)
    (btu/screenshot driver (io/file btu/reporting-dir "created-resource.png"))
    (is (str/includes? (btu/get-element-text driver {:css ".alert-success"}) "Success"))
    (is (= {"Organization" "nbn"
