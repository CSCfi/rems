          ...
          (let [application (get-application-from-api (btu/context-getx :application-id))]
            ...
            (testing "after navigating to the application view again"
              (btu/scroll-and-click [{:css "table.my-applications"}
                                     ...
                                     {:css ".btn-primary"}])
              (wait-page-title (format "Application %s: Test name – REMS" (get-external-id (btu/context-getx :application-id) "alice")))
              (btu/wait-page-loaded)
...
(deftest test-applicant-member-invite-action
  ...
  (btu/with-postmortem
    ...
    (wait-page-title (format "Application %s: test-applicant-member-invite-action – REMS" (get-external-id (btu/context-getx :application-id) "alice")))
...
(deftest test-applicant-member-remove-action
  ...
  (btu/with-postmortem
    (login-as "alice")
    (go-to-application (btu/context-getx :application-id))
    (wait-page-title (format "Application %s: test-applicant-member-remove-action – REMS" (get-external-id (btu/context-getx :application-id) "alice")))
...
(deftest test-handling
  ...
  (btu/with-postmortem
    ...
    (testing "handler should see application after clicking on View"
      (wait-page-title (format "Application %s: test-handling – REMS" (get-external-id (btu/context-getx :application-id) "developer"))))
...
(deftest test-invite-decider
  ...
  (btu/with-postmortem
    (testing "handler invites decider"
      (login-as "developer")
      (go-to-application (btu/context-getx :application-id))
      (wait-page-title (format "Application %s: test-invite-decider – REMS" (get-external-id (btu/context-getx :application-id) "developer")))
...
(deftest test-invite-reviewer
  ...
  (btu/with-postmortem
    (testing "handler invites reviewer"
      (login-as "developer")
      (go-to-application (btu/context-getx :application-id))
      (wait-page-title (format "Application %s: test-invite-reviewer – REMS" (get-external-id (btu/context-getx :application-id) "developer")))
    ...
    (testing "reviewer should see applicant non-private form answers"
      (login-as "carl")
      (go-to-application (btu/context-getx :application-id))
      (wait-page-title (format "Application %s: test-invite-reviewer – REMS" (get-external-id (btu/context-getx :application-id) "carl")))
...
(deftest test-approve-with-end-date
  ...
  (btu/with-postmortem
    ...
    (wait-page-title (format "Application %s: test-approve-with-end-date – REMS" (get-external-id (btu/context-getx :application-id) "developer")))
    (testing "approve"
      (btu/scroll-and-click :approve-reject-action-button)
      (is (btu/eventually-visible? :comment-approve-reject))
      (btu/fill-human :comment-approve-reject "this is a comment")
...
(deftest test-conditional-field
  (btu/with-postmortem
    ...
    (testing "fill in application"
      ...
      (login-as "alice")
      (go-to-application (btu/context-getx :application-id))
      (wait-page-title (format "Application %s – REMS" (get-external-id (btu/context-getx :application-id) "alice")))
